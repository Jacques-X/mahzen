...
import { 
  ...
  Activity, Cpu, MemoryStick, Zap, XCircle
} from 'lucide-react';
...
// --- Info View (Existing) ---
const InfoView = ({ isRoot, diskStats, folderStats, isScanning, onRescan }: any) => {
              ...
              <div className="relative w-40 h-20 overflow-hidden mb-3 flex justify-center">
                 <svg viewBox="0 0 100 50" className="w-full h-full">
                   ...
                   {segments.map((seg: any, i: number) => (
                       <path key={i} d={seg.path} fill="none" stroke={seg.colorHex} strokeWidth="10" strokeLinecap="butt" className="transition-all duration-700 ease-out" />
                   ))}
                 </svg>
              </div>
...
// --- Activity View ---
const ActivityView = ({ systemStats, onKillProcess }: { systemStats: SystemStats | null, onKillProcess: (pid: number) => void }) => {
    if (!systemStats) return <div className="text-center text-gray-400 text-sm mt-10">Loading activity...</div>;

    const formatMem = (bytes: number) => (bytes / 1024 / 1024 / 1024).toFixed(1) + " GB";
    ...
    return (
        <div className="flex flex-col h-full animate-in fade-in duration-500">
             ...
             <div className="bg-gray-50 rounded-2xl p-4 mb-3 border border-gray-100 flex-shrink-0">
                 <div className="flex items-center justify-between mb-2">
                     <div className="flex items-center space-x-2">
                         <div className="p-1.5 bg-red-100 rounded-lg text-red-500"><Cpu size={14} /></div>
                 ...
                 <div className="w-full h-1.5 bg-gray-200 rounded-full overflow-hidden">
                     <div className="h-full bg-red-500 rounded-full transition-all duration-500" style={{ width: `${Math.min(systemStats.cpu_usage, 100)}%` }}></div>
                 </div>
             </div>
             ...
             <div className="bg-gray-50 rounded-2xl p-4 mb-4 border border-gray-100 flex-shrink-0">
                 <div className="flex items-center justify-between mb-2">
                     <div className="flex items-center space-x-2">
                         <div className="p-1.5 bg-blue-100 rounded-lg text-blue-500"><MemoryStick size={14} /></div>
                         <span className="text-xs font-medium text-gray-600">Memory</span>
                     </div>
                     <span className="text-xs font-bold text-gray-800">{formatMem(systemStats.used_memory)} / {formatMem(systemStats.total_memory)}</span>
                 </div>
                 <div className="w-full h-1.5 bg-gray-200 rounded-full overflow-hidden">
                     <div className="h-full bg-blue-500 rounded-full transition-all duration-500" style={{ width: `${memPercent}%` }}></div>
                 </div>
             </div>
             ...
             <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 -mr-2 min-h-0">
                 <div className="space-y-1 pb-2">
                     {systemStats.top_processes.map((proc) => (
                         <div key={proc.pid} className="group flex items-center justify-between p-2 hover:bg-red-50/50 rounded-lg transition-colors border border-transparent hover:border-red-100 relative">
                             <div className="flex items-center space-x-3 overflow-hidden">
                                 ...
                                 <div className="flex flex-col min-w-0">
                                     <span className="text-xs font-bold text-gray-700 truncate max-w-[100px]" title={proc.name}>{proc.name}</span>
...
const RightSidebar = React.memo(({ 
    width, onStartResize, folderStats, diskStats, currentPath, onRescan, 
    isScanning, systemStats, onKillProcess, 
    activeTab, onTabChange // New Props
}: any) => {
  const isRoot = currentPath === "/";

  return (
    <div style={{ width, transition: 'width 0.3s ease-in-out' }} className="relative flex-shrink-0 flex-col hidden lg:flex group">
      ...
      <div className="bg-white h-full rounded-3xl shadow-sm border border-gray-100 flex flex-col p-6 overflow-hidden">
        ...
        <div className="flex bg-gray-100 p-1 rounded-xl mb-4 shrink-0">
            <button 
                ...
                className={`flex-1 flex items-center justify-center space-x-1 py-1.5 rounded-lg text-xs font-medium transition-all ${activeTab === 'info' ? 'bg-white shadow-sm text-gray-800' : 'text-gray-400 hover:text-gray-600'}`}
            >
                <span>Info</span>
            </button>
            <button 
                onClick={() => onTabChange('activity')}
                className={`flex-1 flex items-center justify-center space-x-1 py-1.5 rounded-lg text-xs font-medium transition-all ${activeTab === 'activity' ? 'bg-white shadow-sm text-gray-800' : 'text-gray-400 hover:text-gray-600'}`}
            >
...
export default function App() {
  ...
  const [savedInfoWidth, setSavedInfoWidth] = useState(230); // Remember preferred info width
  const [activeRightTab, setActiveRightTab] = useState<'info' | 'activity'>('info');
  ...
  const resize = useCallback((e: MouseEvent) => {
    ...
    if (isResizingRight) {
      ...
      // If user is resizing while in Info mode, save this as their preference
      if (activeRightTab === 'info') {
          setSavedInfoWidth(newWidth);
      }
    }
  }, [isResizingLeft, isResizingRight, activeRightTab]);
  ...
  const handleRightTabChange = (tab: 'info' | 'activity') => {
      if (tab === activeRightTab) return;

      if (tab === 'activity') {
          // Switching to Activity: Grow to 350px
          // (We already saved info width in the resize handler or state)
          setRightWidth(350); 
      } else {
          // Switching back to Info: Restore saved width
          setRightWidth(savedInfoWidth);
      }
